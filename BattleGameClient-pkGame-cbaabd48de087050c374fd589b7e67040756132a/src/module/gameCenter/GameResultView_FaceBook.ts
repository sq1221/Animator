// TypeScript file
class GameResultView_FaceBook extends StateEui {
    ...
    //连胜页面
    private StraightWins: eui.Group;
    private StraightShare: eui.Image;
    private StraightReturn: eui.Image;
    //最好记录页面
    private NewRecord: eui.Group;
    private NewRecordShare: eui.Image;
    private NewRecordReturn: eui.Image;

    //连胜分享页面
    private straightWinSharePanel: eui.Group;
    private StraightWinsPanelNum: eui.BitmapLabel;
    private StraightWinsAvaterGroup: eui.Group;
    private StraightWinsName: eui.Label;
    private StraightWinsPanelScore: eui.Label;

    //最好记录分享页面
    private NewRecordPanelPanel: eui.Group;
    private NewRecordAvaterGroup: eui.Group;
    private NewRecordName: eui.Label;
    private NewRecordPanelScore: eui.Label;

    public constructor() {
        super(GameResultViewSkin_FaceBook);
        App.MessageCenter.addListener(EventMessage.leaderboardSetScore, this.setScoreHandler, this);
    }
    ...
    //本局最后分数
    preScore: number;
    setScoreHandler(data: number) {
        this.preScore = data;
    }
    alterRankList = (callBackFun: Function) => {
        if (this.preScore == undefined) {
            callBackFun();
            return;
        }
        let userdata = FaceBookPlatform.getUserData((userdata) => {
            let score = userdata.score ? userdata.score : this.preScore;
            let winRounds: number = userdata.winRounds ? userdata.winRounds : 0;
            //自己胜利
            if (DataCenter.instance.room.gameResult.winUserId == DataCenter.instance.user.id)
                winRounds++;
            else {
                //输了维护连胜纪录，直接退出走原来逻辑
                winRounds = 0;
                FaceBookPlatform.setUserData(score, winRounds);
                callBackFun();
                return;
            }
            /**
            ...
            if (FaceBookPlatform.gameData.leaderboard) {
                FaceBookPlatform.setLeaderboardScore(this.preScore, () => {
                    FaceBookPlatform.getLeaderBoardScore((faceBookData: FBInstant.LeaderboardEntry) => {
                        // 成绩更好
                        if (faceBookData.getScore() == this.preScore) {
                            this.showNewRecordPanel(winRounds);
                            return;
                        }
                        this.showStraightPanel(score, winRounds);
                    });
                });
            } else {
                this.showStraightPanel(score, winRounds);
    ...
    private NewRecordReturnHandler = () => {
        this.resultInfoFun();
        this.NewRecord.visible = false;
        this.NewRecordReturn.removeEventListener(egret.TouchEvent.TOUCH_TAP, this.NewRecordReturnHandler, this);
        this.NewRecordShare.removeEventListener(egret.TouchEvent.TOUCH_TAP, this.NewRecordShareHandler, this)
    }
    private StraightReturnHandler = () => {
        this.resultInfoFun();
        this.StraightWins.visible = false;
        this.StraightReturn.removeEventListener(egret.TouchEvent.TOUCH_TAP, this.StraightReturnHandler, this);
        this.StraightShare.removeEventListener(egret.TouchEvent.TOUCH_TAP, this.StraightShareHandler, this)
    }

    private NewRecordShareHandler = () => {
        console.log(`分享最好记录`);
        var renderTexture: egret.RenderTexture = new egret.RenderTexture();
        renderTexture.drawToTexture(this.NewRecordPanelPanel);

        let base64 = renderTexture.toDataURL("image/png");
        FaceBookPlatform.share(`${DataCenter.instance.user.name}set his best record.`, base64, this.NewRecordReturnHandler);
    }

    private StraightShareHandler = () => {
        console.log(`分享连胜记录`)
        var renderTexture: egret.RenderTexture = new egret.RenderTexture();
        renderTexture.drawToTexture(this.straightWinSharePanel);

        let base64 = renderTexture.toDataURL("image/png");
        FaceBookPlatform.share(`${DataCenter.instance.user.name}won ${this.StraightWinsPanelNum.text} victories.`, base64, this.StraightReturnHandler);
    }
    /**
    * 弹出连胜页面
    */
    private showStraightPanel(score: any, winRounds: number) {
        FaceBookPlatform.setUserData(score, winRounds);
        console.log(`连胜${winRounds}局`);
        this.resultInfoView.visible = false;
        this.StraightWins.visible = true;
        this.StraightWinsName.text = DataCenter.instance.user.name;
        this.StraightWinsPanelNum.text = winRounds.toString();
        this.StraightWinsPanelScore.text = this.preScore.toString();
        var playerHead1 = new RoleHeadImage(DataCenter.instance.user.imgUrl);
        playerHead1.scaleX = playerHead1.scaleY = 0.85;
        this.StraightWinsAvaterGroup.addChild(playerHead1);
        this.StraightReturn.addEventListener(egret.TouchEvent.TOUCH_TAP, this.StraightReturnHandler, this);
        this.StraightShare.addEventListener(egret.TouchEvent.TOUCH_TAP, this.StraightShareHandler, this);
    }

    /** 
    * 弹出历史最佳页面
    */
    private showNewRecordPanel(winRounds: number) {
        console.log("历史最佳")
        this.resultInfoView.visible = false;
        this.NewRecord.visible = true;
        this.NewRecordName.text = DataCenter.instance.user.name;
        this.NewRecordPanelScore.text = this.preScore.toString();
        var playerHead1 = new RoleHeadImage(DataCenter.instance.user.imgUrl);
        playerHead1.scaleX = playerHead1.scaleY = 0.85;
        this.NewRecordAvaterGroup.addChild(playerHead1);
        this.NewRecordReturn.addEventListener(egret.TouchEvent.TOUCH_TAP, this.NewRecordReturnHandler, this);
        this.NewRecordShare.addEventListener(egret.TouchEvent.TOUCH_TAP, this.NewRecordShareHandler, this);
        FaceBookPlatform.setUserData(this.preScore, winRounds);
    }
    ...
    public show(callBackFun: Function = null): void {
        ...
        if (this.gameOverResultData.userExitFlag == 1 && this.gameOverResultData.winUserId == DataCenter.instance.user.id) {
            ...
            if (!this.gameCallBackFun) {
                // 正常游戏结算
                if (App.CurrGameId == 11)
                    this.alterRankList(this.resultInfoFun);
                else
                    this.resultInfoFun();
            } else {
                // 抛出事件
    ...
    private gameResultStateImgFun(stateIndex: number) {
        if (!GameResultView_FaceBook.gameInnerResult) {
            switch (stateIndex) {
                case 1:
            ...
            egret.Tween.get(self.playerLeaveView)
                ...
                .call(() => {
                    ...
                    if (!self.gameCallBackFun) {
                        ...
                        // 结算页结算
                        if (App.CurrGameId == 11)
                            self.alterRankList(self.resultInfoFun);
                        else
                            self.resultInfoFun();
                    } else {
                        // 抛出事件
                        this.gameCallBackFun();
                        GameResultView_FaceBook.gameInnerResult = true;
                        self.close();
                    }
                })
        } else {
            GameResultView_FaceBook.gameInnerResult = false;
            // 面板状态切换
    ...
    private resultInfoFun = () => {
        if (App.CurrGameId != 4 && App.IsFaceBook) {
    ...
    public dispose(): void {
        ...
        App.MessageCenter.removeListener(EventMessage.leaderboardSetScore, this.setScoreHandler, this);
    }
    ...
    private onOtherGame(): void {
        this.close();
        if (App.IsFaceBook && App.CurrGameId != 4) {
